#!/usr/bin/env bash
# Reproduce the four-arm MAA2C + LBF controlled schedule comparison.
set -euo pipefail

cd "$(dirname "$0")"

PYTHON_BIN="${PYTHON_BIN:-python}"
WANDB_PROJECT="${WANDB_PROJECT:-adr-p0-lbf}"
...
run_one() {
  ...
  case "$arm" in
    ...
    adr)
      ...
      snd=(
        ...
        SND_DIVERSITY_COEF=0.1 SND_ETA_MAX=0.10 SND_WARMUP_RATIO=0.0
        SND_PROACTIVE_RAMP_END=0.10 SND_PROACTIVE_RAMP_POWER=2.0
        ...
        SND_FEEDBACK_MIN_PEAK=0.30 SND_FEEDBACK_MIN_INTERVAL=50
        SND_FEEDBACK_MIN_PROGRESS=0.20
      )
      ;;
    *)
  ...
  echo "Running MAA2C+LBF arm=${arm} seed=${seed}"
  local -a command=(
    env
    ALGO=maa2c
    "WANDB_PROJECT=$WANDB_PROJECT"
    "WANDB_RUN_GROUP=$group"
    "WANDB_RUN_NAME=MAA2C-${arm}-s${seed}"
    "WANDB_DISABLED=${WANDB_DISABLED:-false}"
    SEAC_DISABLE_VIDEO=1
    "${snd[@]}"
    "$PYTHON_BIN" train.py with paper_lbf_maa2c
    "algorithm.seed=$seed"
    algorithm.num_steps=10
    algorithm.num_processes=10
    algorithm.lr=5e-4
