...
class RolloutStorage(object):
    ...
    def insert(
        ...
        if rewards_extrinsic is not None:
            self.rewards_extrinsic[self.step].copy_(rewards_extrinsic)
        else:
            self.rewards_extrinsic[self.step].copy_(rewards)
    ...
    def compute_returns_extrinsic(
        ...
        """Compute returns using only extrinsic rewards (for SEAC cross-agent learning)."""
        if use_proper_time_limits:
            if use_gae:
                self.value_preds[-1] = next_value
                gae = 0
                for step in reversed(range(self.rewards_extrinsic.size(0))):
                    delta = (
                        self.rewards_extrinsic[step]
                        + gamma * self.value_preds[step + 1] * self.masks[step + 1]
                        - self.value_preds[step]
                    )
                    gae = delta + gamma * gae_lambda * self.masks[step + 1] * gae
                    gae = gae * self.bad_masks[step + 1]
                    self.returns_extrinsic[step] = gae + self.value_preds[step]
            else:
                self.returns_extrinsic[-1] = next_value
                for step in reversed(range(self.rewards_extrinsic.size(0))):
                    self.returns_extrinsic[step] = (
                        (
                            self.returns_extrinsic[step + 1] * gamma * self.masks[step + 1]
                            + self.rewards_extrinsic[step]
                        )
                        * self.bad_masks[step + 1]
                        + (1 - self.bad_masks[step + 1]) * self.value_preds[step]
